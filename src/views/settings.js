...
function setupEvents() {
  ...
  const headerPrintBtn = document.getElementById("set-store-header-print-btn");
  if (headerPrintBtn) {
    headerPrintBtn.replaceWith(headerPrintBtn.cloneNode(true));
    document.getElementById("set-store-header-print-btn")?.addEventListener("click", () => {
      const sampleVenta = getSampleVentaData();
      const paperFormat = localStorage.getItem("fonebase_paper_format") || "80mm";
      const paperWidth = paperFormat === "58mm" ? "48mm" : "80mm";
      const em = sampleVenta.emisor;
